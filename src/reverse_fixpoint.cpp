...
template <typename AbstractState> struct Node {
  ...
  /// Check wether this basic block is the return block of its function.
  bool isReturn() const {
...
executeFixpointAlgorithm(Module const &M) {
  ...
  for (int iter = 0; !worklist.empty() and iter < iterations_max; ++iter) {
    ...
    if (node.isReturn()) {
      dbgs(1) << "  Merging function parameters, is return block\n";
    ...
    for (BasicBlock const *basic_block : llvm::predecessors(node.basic_block)) {
      ...
      preds.push_back(state_branched);
    }

    for (BasicBlock const *basic_block : llvm::successors(node.basic_block)) {
      dbgs(3) << "    Merging basic block " << *basic_block << '\n';
    ...
    if (state_new.isBottom) {
      ...
      for (auto it = node.basic_block->rbegin(); it != node.basic_block->rend();
           it++) {
        ...
        if (isa<PHINode>(&inst)) {
          state_new.applyPHINode(*node.basic_block, preds, inst);
    ...
    dbgs(2) << "  State changed, notifying " << succ_size(node.basic_block)
            << (succ_size(node.basic_block) != 1 ? " predecessors\n"
                                                 : " predecessor\n");

    // Something changed and we will need to update the predecessors
    for (BasicBlock const *succ_bb : predecessors(node.basic_block)) {
...
bool optimize(
  ...
  for (auto func = M.begin(); func != M.end(); func++) {
    for (auto bb = func->begin(); bb != func->end(); ++bb) {
      ...
      instructionsToErase.insert(instructionsToErase.end(),
                                 acc.instructionsToErase.begin(),
                                 acc.instructionsToErase.end());
    }
...
bool AbstractInterpretationPass::runOnModule(llvm::Module &M) {
  auto analysisData = executeFixpointAlgorithm<TrueLiveness>(M);
  return optimize<TrueLiveness>(M, analysisData);
}
